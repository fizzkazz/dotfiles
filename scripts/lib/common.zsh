...
preferred_stow_dir() {
  local preferred_repo="$HOME/.dotfiles"
  local preferred_packages_dir="${preferred_repo}/packages"

  if [[ -f "$REPO_DIR/.git" ]] &&
    [[ "$REPO_DIR" != "$preferred_repo" ]] &&
    [[ -d "$preferred_repo/.git" ]] &&
    [[ -d "$preferred_packages_dir" ]]; then
    print -r -- "$preferred_packages_dir"
    return 0
  fi
...
stale_dotfiles_worktree_link_target() {
  local target_path="$1"
  ...
  [[ "$link_target" == *".codex/worktrees/"* ]] || return 1
  [[ "$link_target" == *".dotfiles/packages/${package_name}/${rel_path}" ]] || return 1

  print -r -- "$link_target"
}

cleanup_stale_dotfiles_worktree_links() {
  local stow_dir="$1"
  shift
  ...
  for package_name in "${packages_to_check[@]}"; do
    ...
    while IFS= read -r source_path; do
      ...
      if link_target="$(stale_dotfiles_worktree_link_target "$target_path" "$package_name" "$rel_path")"; then
        log "古い worktree リンクを削除しています: ${target_path} -> ${link_target}"
        rm -f "$target_path"
      fi
    done < <(find "$package_dir" -mindepth 1 \( -type d -o -type f -o -type l \) -print)
  done
}
...
run_stow_links() {
  ...
  if [[ "$stow_dir" != "$STOW_DIR" ]]; then
    log "git worktree から実行されたため、恒久配置の dotfiles を stow ソースに使用します: $stow_dir"
  fi

  cleanup_stale_dotfiles_worktree_links "$stow_dir" "${packages[@]}"
